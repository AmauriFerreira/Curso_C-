...
int partesNavioAcertadas = 0;
bool tiroParaDireita = true;
int cTiroOriginal;
...
{
	//Ja acertei um navio inteiro... Comeco novamente
	if(partesNavioAcertadas == 3)
	{
		partesNavioAcertadas=0;
		computadorAcertou=false;
	}
	
	if(computadorAcertou) //estrategia "inteligente"
	{
		if(cc == 9 && tiroParaDireita) //limite a direita
			tiroParaDireita = false; //tiro a esquerda
		...
		{
			printf("Agua!!\n");
			tiroParaDireita = false; //tentando a esquerda
			cc = cTiroOriginal;
		}
	}
	else // volte a chutar aleatoriamente
	{
		...
		if(mat[lc][cc] == tironavio)
		{
			...
			printf("Acertei!!\n");
			computadorAcertou = true;
			partesNavioAcertadas=1;
			cTiroOriginal = cc;
			if(cc == 9) //tenho que tentar depois a esquerda
				tiroParaDireita = false;
