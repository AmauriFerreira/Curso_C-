...
2) Reimplemente a função procurar de modo recursivo
(particularmente não há nenhuma vantagem nesta alteração,
é só para treinarmos).
*/
...
int procurarRec (char procurado[], struct ficha vetor[],
	int inicio, int num_fichas)
{
   if(inicio >= num_fichas) return -1; //nao existe
   else if(strcmp(procurado, vetor[inicio].nome) == 0)
		return inicio;
   else return procurarRec (procurado, vetor, inicio+1, num_fichas);
}
...
int main()
{
	...
	while(opcao[0] != '5')
	{
		...
		if(opcao[0] == '2') //procurar
		{
			...
			gets(procurado);
			int resposta = procurarRec(procurado, agenda, 0,
fichas_existentes);
			if(resposta == -1)
				printf("\n\nNome nao encontrado\n");
